      ...
      case "offline":
        output = (
          <Text style={styles.subHeading}>
            Na kostrjanc so dźěła abo jedyn tamny problem wobsteji, zo je so
            dyrbjał serwer hasnyć. Čas, hdy serwer so zaso startuje, njemóže so
            paušalnje prajić.
          </Text>
        );
        break;
      ...
      case "pause":
        output = (
          <>
            <Text style={styles.subHeading}>
              Na kostrjanc so dźěła abo jedyn tamny problem wobsteji, zo je so
              dyrbjał serwer hasnyć.
            </Text>
            <Text style={styles.subHeading}>Čas so na {input[1]}trochuje.</Text>
            <Text style={styles.subHeading}>
              Započatk běše w {"\n"}
              {convertTimestampIntoString(parseInt(input[2]))}
            </Text>
          </>
        );
        break;

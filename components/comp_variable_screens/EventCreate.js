...
import { Image, View, ScrollView, StyleSheet, Keyboard, TextInput, Text, Pressable, Alert } from 'react-native'
...
export const EVENT_TAGS = [
    "p16", "p18", "'muttizettel'", "narodniny", "darmotny zastup", "zymne piwo", "shisha", "mjezynarodne",
    "Kóždy je witany!", "dźěło", "kwas", "dj", "catering", "Buisness", "wjesny klub", "alkohol", "njealkoholiske", "toćene piwo", "slěbory kwas", "złoty kwas",
    "18", "20", "30", "33", "40", "50", "60", "100", "přiroda", "zwěrjo", "live hudźba", "priwatne", "Jenož z přeprošenjom", "zhormadnosć", "wiki", "hody", "jutry"
]
...
export const splittArrayIntoNEqualy = (array, amt) => {
    ...
    if (array.length % amt !== 0) for (let i = 0; i < array.length % amt; i++) output[amt - 1].push(array[array.length - 1 - i]);
    return output;
}
...
export default function EventCreate({ navigation }) {
    ...
    const [additionalData, setAdditionalData] = useState(additionalData_PLACEHOLDER);
    ...
    const publish = async () => {
        ...
        set(ref(db, 'events/' + id), {
            ...
            get(child(ref(db), 'users/' + getAuth().currentUser.uid))
                ...
                .finally(async () => {
                    get(child(ref(db), "users/" + getAuth().currentUser.uid + "/isBuisness"))
                        .then(async (snap) => {
                            ...
                            if (!imageUri) {
                                set(ref(db, 'events/' + id + '/eventOptions'), eventOptions).finally(() => {
                                    Alert.alert("Ewent zarjadowany", 'Waš nowy ewent "' + eventData.title + '" je so wuspěšnje zarjadował.', [
                                        {
                                            text: "Ok",
                                            style: "default",
    ...
    useEffect(() => {
        btnPressed = false;
        get(child(ref(getDatabase()), "users/" + getAuth().currentUser.uid + "/isBuisness"))
            .then(snapshot => {
    ...
    const checkIfPublishable = () => {
        if (!(eventData.title.length !== 0 && eventData.description.length !== 0 && eventData.starting && eventData.ending && eventData.starting < eventData.ending))
            ...
            <ScrollView style={ styles.contentContainer } contentContainerStyle={[ styles.contentInnerContainer, { marginTop: -5 } ]} showsHorizontalScrollIndicator={false}
                ...
                <View style={ styles.sectionContainer }>
                    ...
                    />
                        {/* Ending */}
                    <TextInput
                        style={ styles.input } placeholder="Kónc" maxLength={32}
                        multiline={false} numberOfLines={1} placeholderTextColor={"#5884B0"} selectionColor={"#5884B0"}
                        keyboardType="numbers-and-punctuation" keyboardAppearance='dark'
                        autoCapitalize='none' autoComplete={ false }
                        editable onChangeText={ (value) => {
                    ...
                    {/* optional */}
                {
                    userIsBuisness ?
                    <View style={ styles.sectionContainer }>
                        <Text style={ styles.title }>Tute podaća su za konkretne informacije, njedyrbja so ale podać</Text>
                        ...
                        <View style={ styles.sectionContainer } >
                            ...
                            <View style={ styles.inputContainer }>
                                {
                                    arraySplitter(EVENT_TYPES, 3).map((list, listKey) =>
                                        <View key={listKey} style={ styles.typeListContainer }>
                                            {
                                                list.map((type, key) =>
                                                    <SelectableBtn key={key} selected={additionalData.type === EVENT_TYPES.indexOf(type)} title={type}
                                                        style={ styles.typeBtn } onPress={ () => {
                                                            setAdditionalData({
                                                                ...additionalData,
                            ...
                            {/* Thema */}
                        <TextInput
                            style={ styles.input } placeholder="Tema ewenta" maxLength={16}
                            multiline={ false } numberOfLines={1} placeholderTextColor={"#5884B0"} selectionColor={"#5884B0"}
                            keyboardType="default" keyboardAppearance='dark'  value={additionalData.theme}
                            autoCapitalize='words' autoComplete={ false } textContentType="none" 
                            editable onChangeText={ (value) =>  {
                                setAdditionalData({
                                    ...additionalData,
                                    theme: value
                        ...
                        />
                            ...
                            {/* Entrance fee */}
                        <TextInput
                            style={ styles.input } placeholder="Zastupna płaćizna w €" maxLength={16}
                            multiline={ false } numberOfLines={1} placeholderTextColor={"#5884B0"} selectionColor={"#5884B0"}
                            keyboardType="decimal-pad" keyboardAppearance='dark'
                            autoCapitalize='none' autoComplete={ false } textContentType="none" 
                            editable onChangeText={ (value) =>  {
                                setAdditionalData({
                                    ...additionalData,
                                    entrance_fee: value
                        ...
                        />
                            ...
                            {/* Website */}
                        <TextInput
                            style={ styles.input } placeholder="Website k ewenće" maxLength={16}
                            multiline={ false } numberOfLines={1} placeholderTextColor={"#5884B0"} selectionColor={"#5884B0"}
                            keyboardType="web-search" keyboardAppearance='dark'  value={additionalData.website}
                            autoCapitalize='none' autoComplete={ false } textContentType="URL" 
                            editable onChangeText={ (value) =>  {
                                setAdditionalData({
                                    ...additionalData,
                                    website: value
                        ...
                        <View style={ styles.sectionContainer }>
                            ...
                            <Pressable style={ styles.imgOutlineContainer } onPress={openImagePickerAsync} >
                                <View style={ styles.imgContainer } >
                                    {
                                        imageUri !== null ?
                                            <Image source={{ uri: imageUri }} style={[ styles.img, { aspectRatio: imageAspect } ]} resizeMode="cover" /> :
                                            <View style={[ styles.img, styles.imgBorder, { aspectRatio: imageAspect } ]}>
                        ...
                        <View style={ styles.sectionContainer }>
                            ...
                            <View style={ styles.tagsScrollContainer }>
                                <ScrollView showsHorizontalScrollIndicator={false} style={{ width: "100%" }} contentContainerStyle={{ flexDirection: "column" }}
                                    showsVerticalScrollIndicator={false} keyboardDismissMode='on-drag' horizontal bounces 
                                    onScrollBeginDrag={ () => { if (Platform.OS === 'ios') Keyboard.dismiss }}>

                                    {
                                        splittArrayIntoNEqualy(EVENT_TAGS, tagLineAmt).map((line, lineKey) =>
                                            <View key={lineKey} style={ styles.tagLineContainer }>
                                                {
                                                    line.map((tag, key) =>
                                                        <SelectableBtn key={key} selected={additionalData.tags.includes(EVENT_TAGS.indexOf(tag))} title={tag}
                                                            style={ styles.tagBtn } onPress={ () => {
                                                                let t = [].concat(additionalData.tags);
                                                                if (!t.includes(EVENT_TAGS.indexOf(tag))) t.push(EVENT_TAGS.indexOf(tag));
                                                                else t.splice(t.indexOf(EVENT_TAGS.indexOf(tag)), 1);
                                                                setAdditionalData({
                                                                    ...additionalData,
                                                                    tags: t
                                                                });
                        ...
                        <Text style={ styles.subtext }>
                            Jeli chceš za ewent wabować, potom móžeš po wozjewjenju so na Buisness-Dashboard we syći pod dashboard.kostrjanc.de podać.

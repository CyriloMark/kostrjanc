...
export default function EventView({ navigation, route }) {
  ...
  const loadUser = () => {
    ...
    get(child(db, "users/" + creator))
      ...
      .finally(() => {
        get(child(db, "users/" + getAuth().currentUser.uid + "/isAdmin")).then(
          (snapshot) => {
  ...
  const banEvent = () => {
    Alert.alert(
      "Chceš tutón ewent banować?",
      "Chceš woprawdźe ewent banować? Ewent je banowany, wužiwar nic.",
      [
        {
          text: "Ně",
          style: "destructive",
        },
        {
          ...
          onPress: () => {
            ...
            get(
              ...
              .catch((error) =>
                console.log("error EventView getLogs", error.code)
              );
          },
        },
      ]
  ...
  return (
    <View style={styles.container}>
      <CommentsModal
        ...
        close={() => setCommentsVisible(false)}
      ...
      <UserListModal
        close={() => setChecksVisible(false)}
        visible={checksVisible}
        title={"Tute ludźo su tež pódla"}
        userList={checksUserList}
      />
      ...
      >
        {!event.isBanned ? (
          <UserHeader
            ...
            onPress={() =>
              navigation.navigate("ProfileView", { userID: creator })
            }
            user={user}
            userID={creator}
          />
          ...
          <View style={styles.mapContainer}>
            {!event.isBanned ? (
              ...
              <View style={styles.map}>
                <SVG_Basket style={styles.delIcon} fill="#5884B0" />
              </View>
            )}
          </View>
        </View>
        ...
        {event.eventOptions ? <MainSplitLine style={styles.line} /> : null}
        ...
        {event.eventOptions && !event.isBanned ? (
          <View style={{ width: "100%" }}>
            ...
            <Text
              style={[
                styles.titleText,
                { marginBottom: 5, alignSelf: "baseline" },
              ]}
            >
              Přidatne informacije k ewenće
            </Text>
            ...
            {event.eventOptions.website ? (
              <View style={styles.sectionContainer}>
                ...
                <LinkButton
                  ...
                  title={event.eventOptions.website}
                  link={event.eventOptions.website}
                />
              </View>
            ) : null}
              ...
              <View style={styles.sectionContainer}>
                ...
                <View style={styles.adBannerOutlineContainer}>
                  <View style={styles.adBannerContainer}>
                    <Image
                      source={{ uri: event.eventOptions.adBanner.uri }}
                      style={[
                        styles.img,
                        { aspectRatio: event.eventOptions.adBanner.aspect },
                      ]}
                      resizeMode="cover"
                    />
                  </View>
        ...
        <View style={styles.checksContainer}>
          ...
          <Pressable
            style={[
              ...
              {
                backgroundColor: !event.checks.includes(uid)
                  ? "#B06E6A"
                  : "#143C63",
              },
            ]}
          ...
          {/* List */}
          <Pressable
            style={styles.checksListContainer}
            onPress={getChecksUserList}
          >
            <Text style={styles.checksListText}>
              <Text style={{ fontFamily: "Barlow_Bold" }}>
                {event.checks.length}
              </Text>{" "}
              {getCorrectChecksWord(event.checks.length)} pódla
            </Text>
          </Pressable>
        </View>
      </ScrollView>
      ...
      <ViewInteractionsBar
        ...
        onBan={banEvent}
        comment={!event.isBanned}
        report={!event.isBanned && uid != creator}
        ban={!event.isBanned && uid != creator}
      />
    </View>
  );
}

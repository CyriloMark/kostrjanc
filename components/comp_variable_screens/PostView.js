...
export default function PostView({ navigation, route }) {
  ...
  const loadUser = () => {
    ...
    get(child(db, "users/" + creator))
      ...
      .finally(() => {
        get(child(db, "users/" + uid + "/isAdmin")).then((snapshot) => {
  ...
  const deletePost = () => {
    ...
    set(ref(db, "posts/" + postID), null).then(() => {
      get(child(ref(db), "users/" + creator)).then((snapshot) => {
        const data = snapshot.val();
        let a = data["posts"];
  ...
  const banPost = () => {
    Alert.alert(
      "Chceš tutón post banować?",
      "Chceš woprawdźe post banować? Post je banowany, wužiwar nic.",
      [
        {
          text: "Ně",
          style: "destructive",
        },
        {
          ...
          onPress: () => {
            ...
            get(child(ref(db), "users/" + uid + "/isAdmin"))
              ...
              .catch((error) =>
                console.log("error PostView getLogs", error.code)
              );
          },
        },
      ]
  ...
  return (
    <View style={[styles.container]}>
      ...
      >
        ...
        <View
          style={[
            styles.postOutlineContainer,
            { borderColor: !isBusinessPost ? "#143C63" : "#B06E6A" },
          ]}
        >
          <View style={styles.postContainer}>
            {!post.isBanned ? (
              ...
              <View style={styles.img}>
                <SVG_Basket style={styles.pbImageIcon} fill="#5884B0" />
              </View>
            )}
          </View>
        </View>
        ...
        <View style={styles.textContainer}>
          ...
          <Text
            style={[
              styles.titleText,
              { color: !isBusinessPost ? "#5884B0" : "#B06E6A" },
            ]}
          >
            {post.title}
          </Text>
          ...
          {isBusinessPost ? (
            <SVG_Ad style={styles.businessIcon} fill="#B06E6A" />
          ) : null}
        </View>
      </ScrollView>
      ...
      <ViewInteractionsBar
        ...
        onBan={banPost}
        comment={!post.isBanned}
        share={!post.isBanned}
        report={!post.isBanned && uid != creator}
        ban={!post.isBanned && uid != creator}
      />
    </View>
  );
}

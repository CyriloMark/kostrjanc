...
export default function Search({ navigation }) {
  ...
  const fetchUsers = (text) => {
    ...
    get(child(ref(db), "users"))
      ...
      .catch((error) => console.log("error search getUsers", error.code));
  };
  ...
  const getRandomUser = () => {
    ...
    get(child(ref(db), "users"))
      .then((snapshot) => {
        ...
        const filteredPairList = Object.entries(users)
          .filter((i) => !i[1].isBanned)
          .filter((i) => !(i[0] === getAuth().currentUser.uid));

        const randomUser =
          filteredPairList[
            Math.round(lerp(0, filteredPairList.length - 1, Math.random()))
          ];
        setRandomUser({
          id: randomUser[0],
      ...
      .catch((error) =>
        console.log("error getRandomUser getUsers", error.code)
      );
  };
        ...
        {searchResult.length !== 0 ? (
          ...
          <View style={styles.randomUserContainer}>
            ...
            <UserCard
              ...
              onPress={() =>
                navigation.navigate("ProfileView", { userID: randomUser.id })
              }
            />
          </View>
        ) : (

...
export default function Landing({ navigation }) {
    ...
    get(child(ref(db), "users/" + getAuth().currentUser.uid + "/following"))
      ...
      .finally(() => {
        get(child(ref(db), "users/" + getAuth().currentUser.uid + "/follower"))
                ...
                .finally(() => {
                  if (i == combined.length - 1) {
                    ...
                    if (postIds.length > AMT_posts)
                      slicedList = postIds.sort(function(a, b) { return b - a }).slice(0, AMT_posts)
          ...
          .catch(error => console.log("error getNewPost getUsersFollower", error.code))
      })
      .catch(error => console.log("error getNewPost getUsersFollowing", error.code))
  }
                ...
                .finally(() => {
                  if (i === combined.length - 1) {
                    ...
                    if (eventIds.length > AMT_events)
                      slicedList = eventIds.sort(function(a, b) { return b - a }).slice(0, AMT_events)
                    ...
                    getUserUnspecificPosts(AMT_posts - alreadyUsedPosts, userSpecificContent.sort(function(a, b) { return b - a }), slicedList.length);
                  }
              })
            }
          })
  ...
  let getUserUnspecificPosts = (amt, input, alreadyUsedEvents) => {
    ...
    get(child(ref(db), "posts"))
      .then(snapshot => {
        ...
        if (filteredPosts.length <= amt) filteredPosts.forEach(p => output.push(p['id']));
        else if (filteredPosts.length > amt) filteredPosts.sort(function(a, b) { return b - a }).slice(0, amt).forEach(p => output.push(p['id']));
      ...
      .then(snapshot => {
        ...
        if (!snapshot.exists()) {
          if (postsInput.length === 0) {
            ...
            const finalList = safeContent.concat(postsInput.sort(function(a, b) { return b - a }));
        ...
        const filteredEvents = allEvents.filter(e => !showingEvents.includes(e["id"])).filter(e => !e["isBanned"]).filter(e => e["ending"] > Date.now());

        if (filteredEvents.length <= amt) filteredEvents.forEach(e => output.push(e['id']));
        else if (filteredEvents.length > amt) filteredEvents.sort(function(a, b) { return b - a }).slice(0, amt).forEach(e => output.push(e['id']));
  ...
  return (
    <View style={ styles.container } >
      ...
      <ScrollView ref={mainScrollRef} style={ styles.contentContainer } contentContainerStyle={ styles.contentInnerContainer }
        showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} refreshControl={
          ...
          if (isCloseToBottom(nativeEvent)) {
            ...
            if (!refreshing) getUserUnspecificPosts(AMT_posts, postEventList, 0);
          }}} scrollEventThrottle={400} >

        {
          banners ?
          banners.map((data, key) =>
            <BannerCard key={key} style={[ styles.card, { marginBottom: 15 }]} bannerID={data} />
          ) : null
        }
        
        {
          postEventList ?
            postEventList.map((data, key) =>
              data === "_ad" ?
                <View key={key}>
                  <AdCard style={styles.card} press={ ad => ad.targetType === 0 ?
                    navigation.navigate('PostView', { postID: ad.target, isBusinessPost: true }) :
                    navigation.navigate('EventView', { eventID: ad.target }) }/> 
                  <MainSplitLine style={ styles.line } />
                </View> :
                  showingPosts.includes(data) ?
                    <View key={key} >
                      <PostCard style={styles.card} postID={data} onPress={ () => navigation.navigate('PostView', { postID: data, isBusinessPost: false }) } /> 
                      <MainSplitLine style={ styles.line } />
                    </View> :

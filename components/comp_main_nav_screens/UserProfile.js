...
import { View, StyleSheet, Text, ScrollView, RefreshControl, Pressable, Image, KeyboardAvoidingView, Platform, Keyboard, TextInput, Switch } from "react-native";
                    ...
                    for(let i = 0; i < userData.events.length; i++) {
                        get(child(ref(db), "events/" + userData.events[i]))
                            .then((event) => {
                                ...
                                if (!eventData.isBanned && Date.now() < eventData.ending + ONEWEEK) postEventDatas.push(eventData);
    ...
    return (
        <View style={ styles.container } >
            ...
            <UserListModal close={ () => setFollowingVisible(false) } visible={followingVisible} title={"Tutym ludźom ty sćěhuješ"} userList={followingUserList} />
                ...
                {/* EDIT */}
            <Modal onRequestClose={ () => setEditScreenVisible(false) } visible={editScreenVisible} content={  
                <ScrollView ref={editScrollRef} style={{ width: "100%" }} scrollEnabled bounces
                    showsHorizontalScrollIndicator={false} showsVerticalScrollIndicator={false} keyboardDismissMode='on-drag' >

                        {/* Name */}
                    <View style={[ styles_edit.sectionContainer, styles_edit.inputContainer ]}>
                        <TextInput
                            style={ styles_edit.input } placeholder="Wopisaj tebje..." maxLength={512}
                            multiline={false} numberOfLines={1} placeholderTextColor={"#5884B0"} selectionColor={"#5884B0"}
                            keyboardType="default" keyboardAppearance='dark' value={editDataInput.name}
                    ...
                    <View style={ styles_edit.sectionContainer }>
                        <Pressable style={ styles_edit.imgOutlineContainer } onPress={openImagePickerAsync} >
                            <View style={ styles_edit.imgContainer } >
                                <Image source={{ uri: pbImageUri }} style={ styles_edit.img } resizeMode="cover" />
                            </View>
                        </Pressable>
                    </View>
                    ...
                    <View style={ styles_edit.sectionContainer }>
                        <View style={ styles_edit.genderContainer }>
                            ...
                            <Switch style={ styles_edit.switch } value={editDataInput.gender === 0 ? false : true}
                            ...
                            <View key={listKey} style={ styles_edit.ageGroupListContainer }>
                                { list.map((ageGroup, ageGroupKey) => 
                                    <SelectableBtn key={ageGroupKey} selected={editDataInput.ageGroup === ageGroup.id} title={ageGroup.ageGroup}
                                        subTitle={ageGroup.ages} style={ styles_edit.ageBtn } onPress={ () => {
                                            setEditDataInput({
                                                ...editDataInput,
                    ...
                    <View style={[ styles_edit.sectionContainer, styles_edit.inputContainer ]}>
                        <TextInput
                            ...
                            keyboardType="default" keyboardAppearance='dark' value={editDataInput.description}
                        ...
                        {/* Submit */}
                    <Pressable style={ styles_edit.submitBtnContainer } onPress={ overrideUserData } >

...
export default function UserProfile({ navigation }) {
    ...
    const loadUser = () => {
        ...
        get(child(ref(db), "users/" + getAuth().currentUser.uid))
            .then(snapshot => {
                ...
                let userData = {
                    ...data,
                    follower: snapshot.hasChild("follower")
                        ? data["follower"]
                        : [],
                    following: snapshot.hasChild("following")
                        ? data["following"]
                        : [],
                };
                    ...
                    for (let i = 0; i < userData.posts.length; i++) {
                        get(child(ref(db), "posts/" + userData.posts[i]))
                            .then(post => {
                                ...
                                if (
                                    i === userData.posts.length - 1 &&
                                    !hasEvents
                                )
                                    sortArrayByDate(postEventDatas);
                            })
                            .catch(error =>
                                console.log("error posts", error.code)
                            );
                    }
                }
                ...
                if (hasEvents) {
                    for (let i = 0; i < userData.events.length; i++) {
                        get(child(ref(db), "events/" + userData.events[i]))
                            ...
                            .catch(error =>
                                console.log("error events", error.code)
                            );
                    }
                }
            })
    ...
    const getPostsAndEvents = (hasPosts, hasEvents) => {
        ...
        if (hasEvents) {
            get(
                child(ref(db), "users/" + getAuth().currentUser.uid + "/events")
            )
                .then(snap => {
                    if (snap.exists()) {
                    ...
                    Storage.uploadBytes(
                        ...
                        .then(snapshot => {
                            Storage.getDownloadURL(
                                ...
                                .catch(error =>
                                    console.log("error download", error.code)
                                );
                        })
                        .catch(error =>
            ...
            <Modal
                ...
                content={
                    <ScrollView
                        ...
                        <View
                            style={[
                                styles_edit.sectionContainer,
                                styles_edit.inputContainer,
                            ]}>
                            <TextInput
            ...
            <ScrollView
                ...
                <View style={styles.postContainer}>
                    {arraySplitter(postEventList, 2).map((list, listKey) => (
                        <View
                            key={listKey}
                            style={styles.postItemListContainer}>

      ...
      content={
        ...
        >
          <Text style={styles.title}>
            Komentary na {props.type === 0 ? "post" : "ewent"} "{props.title}"
          </Text>

          <View style={styles.inputContainer}>
            <TextInput
              style={styles.input}
              placeholder="Zapodaj twoje měnjenje..."
              maxLength={128}
              multiline
          ...
          {commentsList.length === 0 ? (
            <Text style={styles.hint}>Być přeni kiž komentuje!</Text>
          ) : (
...
export function Comment(props) {
  ...
  const getUserData = () => {
    ...
    get(child(db, "users/" + props.comment.creator)).then((snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
  ...
  return (
    <View style={props.style}>
      <View style={styles_comment.container}>
        {/* User */}
        <Text style={styles_comment.userText}>
          {userName} - {getTime()}
        </Text>

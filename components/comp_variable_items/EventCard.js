                ...
                get(child(ref(db), "users/" + getAuth().currentUser.uid + "/follower"))
                    ...
                    .finally(() => {
                        if (finalList.length > IMGamt) {
                            ...
                            for (let i = 0; i < IMGamt - finalList.length; i++) finalList.push(otherChecks[i]);
                        } else if (finalList.length < IMGamt && d.length <= IMGamt) {
                            let otherChecks = d.filter(i => !finalList.includes(i));
                        ...
                        for (let i = 0; i < finalList.length; i++) {
                            get(child(ref(db), "users/" + finalList[i] + "/pbUri"))
                                ...
                                .catch(error => console.log("error EventCard getUsersPBUri", error.code))
                        }
                            ...
                            (
                                pin != null ?
                                    <MapView style={styles.map} accessible={false} focusable={false} rotateEnabled={false} zoomEnabled={false} pitchEnabled={false} 
                                        initialRegion={ event.geoCords } scrollEnabled={false} onPress={ props.onPress } >
                                        {
                                            !event.isBanned ?
                                                <Marker coordinate={event.geoCords}  /> : null
                                        }
                                    </MapView> : null
                            ) :
